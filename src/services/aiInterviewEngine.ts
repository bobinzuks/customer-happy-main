...
export class AIInterviewEngine {
  ...
  private async analyzeSentiment(text: string): Promise<{score: number, confidence: number, emotions: string[]}> {
    try {
  ...
  private selectOptimalModel(userInput: string, sentimentScore: number): 'groq' | 'openai' {
  ...
  private async callAIModel(model: 'groq' | 'openai', prompt: string): Promise<{content: string}> {
    if (model === 'groq') {
      const response = await this.groqClient.chat.completions.create({
...
Response:`;
  ...
  private getSentimentContext(score: number): string {
    if (score > 0.3) return "POSITIVE - Customer seems satisfied";
    if (score < -0.3) return "NEGATIVE - Customer has concerns";
    return "NEUTRAL - Customer is providing factual feedback";
  }

  private getResponseStrategy(sentimentScore: number): string {
  ...
  private buildConversationContext(history: ConversationMessage[], settings: BusinessSettings): string {
    if (history.length === 0) {
  ...
  private determineNextQuestionType(sentimentScore: number): string {
    if (sentimentScore > 0.5) return 'positive_details';
    if (sentimentScore < -0.3) return 'problem_exploration';
    return 'general_experience';
  }

  private shouldContinueInterview(messageCount: number, sentimentScore: number): boolean {
  ...
  private generateSuggestedActions(sentimentScore: number): string[] {
    const actions = [];
  ...
  async *streamResponse(
    ...
    businessSettings: BusinessSettings,
    conversationHistory: ConversationMessage[] = []
  ): AsyncGenerator<{type: 'partial' | 'complete', content: string, metadata?: any}> {
    try {
      const context = this.buildConversationContext(conversationHistory, businessSettings);
      const sentimentAnalysis = await this.analyzeSentiment(userInput);
      const prompt = this.buildInterviewPrompt(userInput, context, sentimentAnalysis, businessSettings);
