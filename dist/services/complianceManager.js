...
class ComplianceManager {
    ...
    async generateCompliantReviewRequest(businessId, customerId, interviewId, sentimentScore) {
        try {
            ...
            const customer = customerId ? await database_1.prisma.customer.findUnique({
                where: { id: customerId }
            }) : null;
            const reviewRequest = {
            ...
            // Store in database
            await database_1.prisma.reviewRequest.create({
                data: {
                    id: reviewRequest.id,
    ...
    async validateReviewRequest(requestData) {
        const violations = [];
    ...
    generateCompliantMessage(businessName, customerName) {
...
Your feedback helps us improve our service and assists other customers in making informed decisions.`,
    ...
    generateGoogleReviewUrl(business) {
    ...
    generatePrivateFeedbackUrl(businessId) {
        return `${process.env.APP_URL}/feedback/${businessId}`;
    }
    containsSentimentBias(message) {
        const biasKeywords = [
            'if you\'re happy',
            'positive experience',
            'satisfied customers',
        ...
        return biasKeywords.some(keyword => message.toLowerCase().includes(keyword.toLowerCase()));
    }
    containsConditionalIncentive(message) {
        ...
        return incentiveKeywords.some(keyword => message.toLowerCase().includes(keyword.toLowerCase()));
    }
    hasEqualOpportunityLanguage(message) {
        ...
        return equalOpportunityPhrases.some(phrase => message.toLowerCase().includes(phrase.toLowerCase()));
    }
    hasTimingRestrictions(requestData) {
    ...
    async generateComplianceReport(businessId, startDate, endDate) {
        try {
            const reviewRequests = await database_1.prisma.reviewRequest.findMany({
                where: {
                    businessId,
                    sentAt: {
                        gte: startDate,
            ...
            await database_1.prisma.complianceRecord.create({
...
class AuditLogger {
    async logReviewRequest(requestData) {
        try {
