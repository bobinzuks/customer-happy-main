...
async function dashboardRoutes(fastify) {
    // Get KPI data
    fastify.get('/api/dashboard/kpis', async (request, reply) => {
        try {
    ...
    // Get chart data
    fastify.get('/api/dashboard/charts', async (request, reply) => {
        try {
            const { business = 'all', range = 'week' } = request.query;
            const { startDate, endDate } = getDateRange(range);
    ...
    fastify.get('/api/dashboard/export', async (request, reply) => {
        try {
    ...
    fastify.get('/api/dashboard/realtime', async (request, reply) => {
        try {
    ...
    async function getInterviewStats(businessId, startDate, endDate) {
        const whereClause = {
            startedAt: { gte: startDate, lte: endDate },
            ...(businessId !== 'all' && { businessId })
        };
        const [total, completed, previousPeriod] = await Promise.all([
            database_1.prisma.interview.count({ where: whereClause }),
            database_1.prisma.interview.count({
    ...
    async function getReviewStats(businessId, startDate, endDate) {
        const whereClause = {
            sentAt: { gte: startDate, lte: endDate },
            ...(businessId !== 'all' && { businessId })
        };
        const [totalRequests, googleReviews] = await Promise.all([
            database_1.prisma.reviewRequest.count({ where: whereClause }),
    ...
    async function getSentimentStats(businessId, startDate, endDate) {
        const whereClause = {
            startedAt: { gte: startDate, lte: endDate },
            ...(businessId !== 'all' && { businessId }),
            sentimentScore: { not: null }
        };
        const result = await database_1.prisma.interview.aggregate({
            where: whereClause,
    ...
    async function getResponseTimeStats(businessId, startDate, endDate) {
        const whereClause = {
        ...
        const result = await database_1.prisma.conversationMessage.aggregate({
            where: whereClause,
    ...
    async function getSentimentTrendData(businessId, startDate, endDate) {
    ...
    async function getVolumeTrendData(businessId, startDate, endDate) {
        const days = Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
        const labels = [];
        const completed = [];
        const abandoned = [];
    ...
    async function getRecentActivity(businessId, startDate, endDate) {
        ...
        const [recentInterviews, recentReviews] = await Promise.all([
            ...
            database_1.prisma.reviewRequest.findMany({
                where: {
                    sentAt: { gte: startDate, lte: endDate },
                    status: 'completed',
                    ...(businessId !== 'all' && { businessId })
    ...
    async function getExportData(businessId, startDate, endDate) {
    ...
    async function getRealtimeData(businessId, startDate, endDate) {
        const [activeInterviews, recentCompletions, avgResponseTime] = await Promise.all([
            ...
            database_1.prisma.conversationMessage.aggregate({
                where: {
                    timestamp: { gte: startDate, lte: endDate },
                    sender: 'ai',
                    responseTimeMs: { not: null },
